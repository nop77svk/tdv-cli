namespace NoP77svk.TibcoDV.CLI.AST
{
    ...
    internal class CommandCreateResource : IAsyncStatement
    {
        ...
        public async Task Execute(TdvWebServiceClient tdvClient, IInfoOutput output)
        {
            ...
            if (ResourceDDL is AST.FolderDDL folderDDL)
                await ExecuteCreateFolder(tdvClient, IfNotExists, folderDDL, output);
            else if (ResourceDDL is AST.SchemaDDL schemaDDL)
                await ExecuteCreateSchema(tdvClient, IfNotExists, schemaDDL, output);
            else if (ResourceDDL is AST.ViewDDL viewDDL)
